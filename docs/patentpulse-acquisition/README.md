...
This folder is the buyer packet for **PatentPulse** (`theworker02/patentpulse`): measured corpus quality, ingestion throughput, compressed and uncompressed sizes, deduplication and error rates, a reproducible deployment procedure, and the companies for whom an acquisition replaces months of USPTO data engineering.

This cloud agent can push only to Atlas of Knowledge. Apply `patentpulse-code.patch` onto `theworker02/patentpulse` (or copy `upstream/metrics.py` plus the `hf_release.py` empty-split fix) so `python -m patentpulse.metrics` lives in the pipeline repository.
...
| [patentpulse-code.patch](patentpulse-code.patch) | Apply onto `theworker02/patentpulse` |
| [upstream/](upstream/) | Full copies of the pipeline files the patch adds or edits |
